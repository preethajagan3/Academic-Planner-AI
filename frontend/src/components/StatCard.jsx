const StatCard = ({ title, value, icon, color = 'indigo', type = 'default', completedCount = 0, totalCount = 0 }) => {
  const colorMap = {
    cyan: 'bg-cyan-500',
    green: 'bg-emerald-500',
    purple: 'bg-purple-500',
    orange: 'bg-orange-500',
    indigo: 'bg-emerald-600',
    emerald: 'bg-emerald-600',
  };

  const accentColor = colorMap[color] || 'bg-indigo-500';
        ...
        return (
          <div className="space-y-2">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-900 uppercase tracking-widest">
              <span>Completed</span>
              <span className="text-emerald-600">{completedCount}/{totalCount}</span>
            </div>
        ...
        return (
          <div className="space-y-2">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-900 uppercase tracking-widest">
              ...
              <span className={`${percentage >= 70 ? 'text-emerald-600' : percentage >= 40 ? 'text-amber-600' : 'text-slate-900'}`}>
                {percentage}%
              </span>
            </div>
            <div className="flex items-center gap-3">
              <svg className="w-14 h-14 -rotate-90 flex-shrink-0">
                ...
                <circle
                  ...
                  cy="28"
                  r={radius}
                  stroke={percentage >= 70 ? '#10b981' : percentage >= 40 ? '#f59e0b' : '#94a3b8'}
                  strokeWidth="5"
                  fill="none"
